...
namespace Platforms.Android
{
	internal class CustomShellItemRenderer : ShellItemRenderer
	{
			...
			Bundle savedInstanceState)
		{
			...
			if (Context is not null && ShellItem is OctopusTabBar { CenterViewVisible: true } tabbar)
			{
				...
				{
					BottomMargin = 100, Width = middleViewSize, Height = middleViewSize
				};
				var middleView = new Button(Context);
				middleView.LayoutParameters = middleViewLayoutParams;
				middleView.Click += delegate { tabbar.CenterViewCommand?.Execute(null); };
				...
				if (tabbar.CenterViewBackgroundColor is not null)
				{
					...
					backgroundDrawable.SetColor(
						tabbar.CenterViewBackgroundColor.ToPlatform(Colors.Transparent));
					middleView.SetBackground(backgroundDrawable);
				}
				tabbar.CenterViewImageSource?.LoadImage(
					Application.Current!.MainPage!.Handler!.MauiContext!, result =>
					{
